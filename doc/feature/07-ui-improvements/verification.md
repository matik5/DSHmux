...
| R9 — DSH sidebar visibility | ✅ runtime-DOM/source/automated, live follow-up | Overflow contains one localized toggle. Missing webview state defaults to hidden. Runtime DOM measurement confirmed the DSH grid as `56px 700px 0px`; applying `visibility:hidden` to its first occupant plus a `0px` first track kept chat in column two and expanded its measured box from x=56/width=700 to x=0/width=756. The regression test forbids `display:none` on that occupant. |
