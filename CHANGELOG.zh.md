...
### 变更
- Doctor 默认在版本化目录中安装固定提交的 `matik/dsh-patches-0.1.7-rc.1` 源码构建，其模型请求图片使用兼容 JPEG；仍可选择官方 `@deepseek-ai/dsh@0.1.7-rc.1` npm 包。
- DSHmux 菜单现在可直接选择该补丁构建，即使旧版 DSH 仍可正常运行。显式升级验证成功后会选用新程序并更新 `dshmux.dshPath`。

### 修复
- 内嵌 WebView 现在可加载 DSH 0.1.7-rc.1 使用相对 URL 的插件预加载、启动条目和批次，并正确解析新增的深色 favicon。
- 源码检出在配置了启动较慢的 MCP 服务时可等待最多 120 秒；其他 DSH 安装仍使用 30 秒的启动超时。

## [0.4.7] - 2026-09-13

### 变更
- 首次设置现在将官方固定版本 `@deepseek-ai/dsh@0.1.5-rc.2` 安装到 DSHmux 管理的版本化存储中。普通安装不再依赖分叉仓库、Git、pnpm、源码构建、管理员权限或用户的全局 npm 前缀。
- DSH Doctor 现在负责安装和修复：先确认准确的软件包和目标路径，再显示可取消的进度与有界日志，并且只有在验证 CLI 版本完全一致后才会信任该安装。
- 启动器不再显示大型设置面板和兼容性提示，只保留一个紧凑的 **缺少依赖 · 修复** 操作。现有有效的全局、npx 缓存、源码和自定义 DSH 安装仍受支持。
- Node 仅作为 DSH 的实际运行/安装前提进行检查。提示现在符合 DSH 0.1.5-rc.2 的官方要求：Node 22.19+（22.x）或 Node 24+。

### 修复
- Doctor 重复刷新时可能在启动器中追加重复的缺失依赖链接，因为动态重建的按钮丢失了清理所需的 ID。现在 Fix 控件是唯一的静态元素，只切换可见性。
- 托管修复现在复用 Doctor 已解析的 Node/npm 环境，在 Windows 上以结构化参数保留包含空格的路径且不经过命令 shell，并在取消后等待所属 npm 进程真正关闭。
- 版本固定的托管安装不再显示无法替换当前托管副本的全局/npx 升级命令。

## [0.4.4] - 2026-09-05

### 变更
- 启动器的 DSH 兼容性标记现在以 dsh `0.1.2-rc.1`（原为 `0.1.2-alpha.2`）为基准，已针对本版本的会话切换、新建会话和内嵌客户端流程完成验证。
- 移除了用于跟踪会话切换问题的临时诊断 JSONL 日志（`dshmux-diag.jsonl`）；修复已在 0.4.3 发布，不再需要这些诊断。

## [0.4.3] - 2026-09-05

### 修复
- 侧边栏对话：切换到其他会话后，内嵌 DSH UI 可能显示为空（工作区和会话列表无法加载）。原因：前一页面的 WebSocket 中继连接在文档替换后仍然存在，阻塞了新页面的流连接 ID。DSHmux 现在会在替换内嵌文档之前丢弃已中继的套接字（编辑器标签页同样适用）。
- 当 DSH 服务器无响应时，“新建会话”可能静默挂起；API 请求现在会在 15 秒后超时并显示警告，而不是无限等待。
- 启动（或重启）时，若 VS Code 重启后多个 DSH 实例同时启动，IDE 工作区的会话预设现在会重试瞬时的工作区会话失败，而不是回退到全局最近使用的工作区。
